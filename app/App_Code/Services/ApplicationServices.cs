...
namespace yoshuwa.Services
{
    ...
    public class ApplicationServicesBase
    {
        ...
        {
            yoshuwa.Services.ApplicationServices.Initialize();
        }
        ...
        public virtual JObject UserSettings(Page p)
        {
            ...
            settings["membership"]["enabled"] = AuthorizationIsSupported;
        ...
        public static void RegisterStandardMembershipAccounts()
        {
            if (AuthorizationIsSupported)
            {
                ...
                if (admin == null)
                {
                    ...
                    admin = Membership.CreateUser("admin", "admin123%", "admin@yoshuwa.com", "ASP.NET", "Code OnTime", true, out status);
                }
                if (user == null)
                {
                    MembershipCreateStatus status;
                    user = Membership.CreateUser("user", "user123%", "user@yoshuwa.com", "ASP.NET", "Code OnTime", true, out status);
                }
                if (!Roles.IsUserInRole(admin.UserName, "Users"))
                    Roles.AddUserToRole(admin.UserName, "Users");
                if (!Roles.IsUserInRole(user.UserName, "Users"))
                    Roles.AddUserToRole(user.UserName, "Users");
                if (!Roles.IsUserInRole(admin.UserName, "Administrators"))
                    Roles.AddUserToRole(admin.UserName, "Administrators");
                for (var i = 1; (i <= 10); i++)
                {
                    var userName = string.Format("test{0:00}", i);
                    var testUser = Membership.GetUser(userName);
                    if ((testUser != null) && testUser.IsLockedOut)
                    	testUser.UnlockUser();
                    if (testUser == null)
                    {
                        MembershipCreateStatus status;
                        testUser = Membership.CreateUser(userName, string.Format("test{0:00}123%", i), string.Format("{0}@yoshuwa.com", userName), "ASP.NET", "Code OnTime", true, out status);
                    }
                    if (!Roles.IsUserInRole(testUser.UserName, "Users"))
                    	Roles.AddUserToRole(testUser.UserName, "Users");
                }
            }
        }
