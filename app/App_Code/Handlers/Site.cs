...
namespace yoshuwa.Handlers
{
    ...
    public class SiteBase : yoshuwa.Web.PageBase
    {
        ...
        public static string Copyright
        {
            get
            {
                return "&copy; 2020 yoshuwa. ^Copyright^All rights reserved.^Copyright^";
            }
        }
        ...
        protected virtual void InitializeSiteMaster()
        {
            ...
            var themePath = Server.MapPath("~/App_Themes/yoshuwa");
            if (Directory.Exists(themePath))
            	foreach (var stylesheetFileName in Directory.GetFiles(themePath, "*.css"))
                {
                    ...
                    if (!(fileName.Equals("_Theme_Azure.css")))
                    {
                        ...
                        link.Href = ("~/App_Themes/yoshuwa/" + fileName);
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            content = InjectSessionStatusClient(content);
            content = ResolveAppUrl(content);
            ApplicationServices.CompressOutput(Context, content);
        }

        private string InjectSessionStatusClient(string content)
        {
            if (content.IndexOf("SessionStatus.ashx", StringComparison.OrdinalIgnoreCase) >= 0)
                return content;
            var scripts = "\r\n<script>" +
                "(function(){var root=window.__baseUrl||'/';function login(){window.location.href=root+'Login.ashx?ReturnUrl='+encodeURIComponent(window.location.pathname+window.location.search);}" +
                "function poll(){var xhr=new XMLHttpRequest();xhr.open('GET',root+'SessionStatus.ashx?_='+new Date().getTime(),true);xhr.onreadystatechange=function(){if(xhr.readyState===4){if(xhr.status===401||xhr.status===403)login();else if(xhr.status===200&&xhr.responseText.indexOf('forced')>=0)login();}};xhr.send();}" +
                "window.setInterval(poll,5000);" +
                "})();" +
                "</script>";
            return Regex.Replace(content, "\\s*</body>", scripts + "\r\n</body>", RegexOptions.IgnoreCase);
        }
