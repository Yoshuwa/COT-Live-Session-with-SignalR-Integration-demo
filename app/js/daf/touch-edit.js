...
* Copyright 2018-2020 Code On Time LLC; Licensed MIT; http://codeontime.com/license
*/
...
(function () {
        ...
        position: function (page) {
            ...
            if (changePosition) {
                ...
                if (overlayDisplayMode) {
                    ...
                    if (fieldElem.is('.app-field-type-bool'))
                        elemRect = _edit._frame[0].getBoundingClientRect();
                    ...
                    if (!positioned) {
                        ...
                        if (button.length) {
                            ...
                            button.css('top', (fakeInner.outerHeight() - button.outerHeight()) / 2);
                            fakeInner.remove();
                        }
                    }
                }
    ...
    $(document).on('dataitemselect.dataview.app', function (e) {
        ...
        if (e.namespace === 'app' && !_edit._active) {
            ...
            else if (_edit.move({ direction: direction, originalEvent: originalEvent }))
                return false;
        }
            ...
            if ((key.length === 1 || key.match(/Backspace|Del|Delete/)) && !(originalEvent.ctrlKey || originalEvent.altKey || originalEvent.metaKey)) {
                ...
                if (!_input.barcode(':input'))
                    if (key.match(/Del|Delete/))
                        ...
                        _input._buffer = key;
                        originalEvent.preventDefault();
                        _keyboardActivateTimeout = setTimeout(function () {
                            if (!_input.barcode(':input'))
                                _edit.activate(); // do not activate in the "keydown" handler
                        }, 32); // this is generally slower the the speed of barcode scan
                    }
